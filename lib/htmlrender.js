...
// allows the {{}} placeholders to do their job in the templates/
const fillData = (template, placeholder, value) => {
  const pattern = new RegExp('{{ ' + placeholder + ' }}', 'gm');
  return template.replace(pattern, value);
};
//push inquirer data to rendermanager, engineer, etc.
const render = (groups) => {
  const html = [];
...
//sending the information to the templates.
...
const renderIntern = (intern) => {
  let template = fs.readFileSync(
    path.resolve(templateDir, '../template/interntemp.html'),
    'utf8'
  );
